...
public class PaymentService {
    ...
    public String paymentCircuitBreaker(Long id){
        if(id < 0){
            throw new RuntimeException("传入的id不能为负"+UUID.randomUUID().toString()+"  id"+id);
        }
        return "线程："+Thread.currentThread().getName()+UUID.randomUUID().toString()+"  id"+id;
    }

    public String paymentCircuitBreakerFallback(Long id){
        return "熔断fallback  id不能为负，id:"+id+"   "+UUID.randomUUID().toString();
    }
}
